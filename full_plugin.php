...
class EventPost {
	...
	public function __construct() {
		...
		include_once ($inc_path . 'icons.php');
		include_once ($inc_path . 'taxonomies.php');

		$this->DashIcons = new EventPost\DashIcons();
		$this->Settings = new EventPost\Settings($this->DashIcons);
		$this->Categories = new EventPost\Categories($this->DashIcons);
	}
	...
	public function widgets_init(){
		...
		$this->Categories->add_fields_to_taxonomies($this->settings['posttypes']);
		...
		if($color && !empty($color)){
			...
			if($check_taxo){
				foreach(event_post_get_all_terms($post_id) as $term){
					$taxo_color = $this->Categories->get_taxonomy_color($term->term_id);
					if($taxo_color){
	...
	public function get_post_icon($post_id, $default = false, $check_taxo = false) {
		...
		if($icon && !empty($icon)){
			...
			if($check_taxo){
				foreach(event_post_get_all_terms($post_id) as $term){
					$taxo_icon = $this->Categories->get_taxonomy_icon($term->term_id);
					if($taxo_icon){
...
<?php
	...
	public function print_categories($post=null, $context='') {
		...
		if ($categories) {
			...
			foreach ($categories as $category) {
				...
				$color = $this->Categories->get_taxonomy_color($category->term_id);
				if ($color != '' && $color) {
	...
	$ep_settings = $this->settings;
		...
		if (sizeof($events) > 0) {
			...
			if($id == "event_timeline"){
				$prev_arrow = '<div class="previous">'.__('« Previous Events', 'event-post')."</div>";
				$next_arrow = '<div class="next">'.__('Next Events »', 'event-post')."</div>";
				if($nb != 0){
					$item_child_style = 'width : '.((100/$nb) - 2).'%;';
				}
				$attributes .= ' data-nb="'.$nb.'" data-filter="'.http_build_query($atts).'" ';
			}
	...
	public function icon_color_fields($item_id, $meta_color,$value_color,$meta_icon,$value_icon ){
		...
		<div class="eventpost-misc-pub-section event-color-section">
				...
				type="text"
				name="<?php echo $meta_color; ?>"
				value="<?php echo $value_color; ?>"
				id="color-field<?php echo $item_id; ?>"/>

			<select style="font-family : dashicons;" class="eventpost-iconpicker" name="<?php echo $meta_icon; ?>">
				<option value=""><?php echo __('None','event-post') ?></option>
				<?php
					foreach($this->DashIcons->icons as $class => $unicode){
						?>
							<option value="<?php echo $class ?>" <?php selected($value_icon, $class, true); ?>>&#x<?php echo $unicode ?>; <?php echo $class ?></option>
						<?php
					}
				?>
			</select>
			<div class="custom-marker-container">
				<p><?php echo __('An image was found in your custom folder for this color', 'event-post')?> <span class="color-hex"></span> </p>
   ...
   function quick_edit( $column_name, $post_type, $bulk=false) {
	...
	function inline_field_color($html, $fieldname, $bulk){
		if($fieldname==$this->META_COLOR){
			...
			<input class="eventpost-inline-colorpicker eventpost-inline-'.$fieldname.' '.($bulk?'is-bulk':'no-bulk').'" type="text" name="'.$fieldname.'" >';
		}
		return $html;
	}
	...
	public function save_postdata($post_id) {
		...
		// Clean color or no color
		if (false !== $icon = filter_input(INPUT_POST, $this->META_ICON, FILTER_SANITIZE_STRING)) {
			update_post_meta($post_id, $this->META_ICON, $icon);
		}
	 ...
	 * Should be used via AJAX
	 *
	 * outputs JSON
	 * @return void
	 */
	function save_bulkdatas() {
		$current_post_type = isset($_POST['post_type']) ? $_POST['post_type'] : 'post';
		if (in_array($current_post_type, $this->settings['posttypes'])) {
			$post_ids = (!empty($_POST['post_ids']) ) ? $_POST['post_ids'] : array();
	...
	public function ajaxGetNextPage(){
		...
		if(isset($_POST['query'])){
			...
			if(isset($_POST['paged'])){
				...
				$response['message'] = __('Page number missing');
			}
			...
			if($html == ""){
				$response['message'] = __('No more to load');
			}else{
			...
			$response['message'] = __('Query missing');
		}
